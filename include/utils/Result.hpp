...
template<class T>
constexpr static auto is_result_v = is_result<T>::value;
...
public:
    ...
    constexpr auto flatMap(Func&& f) &
    {
        ...
        static_assert(is_result_v<ReturnType>,
    ...
    constexpr auto flatMapError(Func&& f) &
    {
        ...
        static_assert(is_result_v<Result>,
                      "F must return a result");
...
public:
    ...
    constexpr auto flatMapError(Func&& f) &
    {
        using ReturnType = std::invoke_result_t<Func, Err>;
        static_assert(is_result_v<ReturnType>,
                      "F must return a result");

        static_assert(std::is_same_v<typename ReturnType::value_type, Err>,
...
auto traverse(const std::vector<T>& vec, F&& func)
    ...
    -> std::enable_if_t<!std::is_void<typename std::invoke_result_t<F, T>::result_type>::value &&
                       is_result_v<std::invoke_result_t<F, T>>,
...
// clang-format on 
{
    ...
    static_assert(is_result_v<FuncRet>,
